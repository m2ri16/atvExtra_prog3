...
public class PilhaDeCaixas {
    ...
    public boolean empilhar(Caixa novaCaixa) {
       ...
       for (Caixa caixa : pilhaDeCaixas) {
            somatorioDasAlturas += caixa.getAlturaCm(); 
       }

       if (somatorioDasAlturas + novaCaixa.getAlturaCm() <= this.alturaMax) {
    ...
    public void exibirDados(){
        for (int i = 0; i < pilhaDeCaixas.size(); i++) {
            ...
            System.out.println(" Altura: " +caixa.getAlturaCm() + " cm");
            System.out.println(" Peso: " + caixa.getPesoKg() + " kg");
        }
